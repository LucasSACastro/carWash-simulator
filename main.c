...
void sim(unsigned int numero_clientes, FILE *fout) {
	...
	for(cliente=1; cliente<=numero_clientes; cliente++) {
            ...
            servico.inicio= (servico.fim>chegada.relogio ? servico.fim : chegada.relogio);
            tempo_cliente.fila = servico.inicio-chegada.relogio;
            ocio_operador.instance = servico.inicio-servico.fim;
            ...
            printf(
                "Cliente %3d: %3u | %3u | %3u | %3u | %3u | %3u | %3u\n\n\n",
                cliente, chegada.delay, chegada.relogio, servico.duracao, tempo_cliente.fila,
                servico.fim, tempo_cliente.total, ocio_operador.instance
            );
        }

        printf(
            "Somas:           |     | %3u | %3u |     | %3u | %3u\n",
            servico.sum_duracao, tempo_cliente.sum_fila, tempo_cliente.sum_totais, ocio_operador.sum
        );
